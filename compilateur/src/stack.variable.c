...
address temp_pop() {
    if (var_head == 0x00)
        yyerror("il n'y a pas de variable");
    var_head--;
...
memory_address var_get_or_temp_push(label name) {
    ...
    if (name != NULL)
        ...
        if (visibility >= 1)
            out.isLocal = 1; // on peut avoir des temps globaux
        else
            out.isLocal = 0;
    }
...
void add_visibility() {
    if (visibility == MAX_ADDRESS / 2 - 1)
        yyerror("can't add visibility level");
    if (visibility == 0)
        offset_function = var_head;
...
void remove_visibility() {
    if (visibility == 0)
        yyerror("can't remove visibility level");
    if (visibility == 1)
        offset_function = 0;
    visibility--;
